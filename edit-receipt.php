            ...
            <div class="container text-right hero" style="margin-top: 10px;">
                <div class="row text-right" style="direction: rtl;" direction="rtl;">       
                    <div class="col text-right" direction="rtl;">                         
                        <div class="float-left image-holder" data-bs-hover-animate="pulse" style="background-image: url(&quot;assets/img/addreceipt.jpg&quot;);width: 607px;height: 600px;padding: 0 0 20px;background-size: cover;background-color: #e8ba61;"></div>
...
<script type="text/javascript">
  ...
  $.get( "api.php?action=get_receipt_info&id="+id+"&token="+token, function( data ) {
        var obj = $.parseJSON(data);
        if (obj[0]['type']!="1") {
                    window.location.href = "minimall.php";
        }
         $(".price").val(obj[0]['price']);
        $(".notes").val(obj[0]['notes']);
        client_id=obj[0]['client_id'];
        oldbalance=parseInt(obj[0]['price']);
        console.log("22oldbalance="+oldbalance);
        console.log(obj);
...
$('#myform').submit(function () {
 return false;
    ...
    function check() {
        // body...
            $.get( "api.php?action=update_receipt&price="+price+"&notes="+notes+"&id="+id+"&token="+token, function( data ) {
        var obj = $.parseJSON(data);
 ...
 function get_balance() {
            // body...           
        $.get( "api.php?action=get_balance&client_id="+client_id+"&token="+token, function( data ) {
        var obj = $.parseJSON(data);
        console.log("old_balance="+obj["balance"]);
        oldbalancet=parseInt(obj["balance"]);
        console.log("oldbalance="+oldbalance);
        newbalance=parseInt($('.price').val());

        newbalance=newbalance-oldbalance;
                console.log("beforenewbalance="+newbalance);

        oldbalancet-=newbalance;
      ...
      function update_balance(x) {
            // body...           
        $.get( "api.php?action=update_balance&balance="+newbalance+"&client_id="+client_id+"&token="+token, function( data ) {
        var obj = $.parseJSON(data);
